...
public class AddressService implements IAddressService {
	...
	public void validate(AddressCreationContainer addressCreationContainer,
			SecurityContext securityContext) {
		baseclassNewService.validateCreate(addressCreationContainer,
				securityContext);
	...
	public boolean updateAddressNoMerge(Address address,
		...
		boolean update = baseclassNewService.updateBaseclassNoMerge(
				creationContainer, address);

		if (creationContainer.getFloor() != null
				&& !creationContainer.getFloor().equals(
						address.getFloorForAddress())) {
			address.setFloorForAddress(creationContainer.getFloor());
			update = true;
		}

		if (creationContainer.getNumber() != null
				&& !creationContainer.getNumber().equals(address.getNumber())) {
			address.setNumber(creationContainer.getNumber());
			update = true;
		}
		if (creationContainer.getNeighbourhood() != null
				&& (address.getNeighbourhood() == null || !creationContainer
						.getNeighbourhood().getId()
						.equals(address.getNeighbourhood().getId()))) {
			address.setNeighbourhood(creationContainer.getNeighbourhood());
			update = true;
		}
		if (creationContainer.getStreet() != null
				&& (address.getStreet() == null || !creationContainer
						.getStreet().getId()
						.equals(address.getStreet().getId()))) {
			address.setStreet(creationContainer.getStreet());
			update = true;
		}

		if (creationContainer.getZipCode() != null
				&& !creationContainer.getZipCode().equals(address.getZipCode())) {
	...
	public Address createAddress(AddressCreationContainer creationContainer,
			com.flexicore.security.SecurityContext securityContext) {
		Address address = createAddressNoMerge(creationContainer,
				securityContext);
		repository.merge(address);
		return address;
	}
	...
	public Address createAddressNoMerge(
		...
		Address address = new Address(creationContainer.getName(),
				securityContext);
		...
		try {
			...
			if (response.getStatus() == 200) {
				...
				try {
					...
					Country israel = countryService
							.listAllCountries(securityContext, countryFiltering)
							.parallelStream().findFirst().orElse(null);
					if (israel == null) {
						israel = countryService.createCountry(
								new CountryCreationContainer()
										.setName("Israel"), securityContext);
					}
					Map<String, City> existingCities = cityService
							.listAllCities(securityContext, new CityFiltering())
							.parallelStream()
							.filter(f -> f.getExternalId() != null)
							.collect(
									Collectors.toMap(f -> f.getExternalId(),
											f -> f, (a, b) -> a));
					Map<String, Map<String, Street>> existingStreets = streetService
							.listAllStreets(securityContext,
									new StreetFiltering())
							.parallelStream()
							.filter(f -> f.getExternalId() != null
									&& f.getCity() != null
									&& f.getCity().getExternalId() != null)
							.collect(
									Collectors.groupingBy(f -> f.getCity()
											.getExternalId(), Collectors.toMap(
											f -> f.getExternalId(), f -> f, (a,
													b) -> a)));
					...
					for (StreetEntry streetEntry : data) {
						...
						CityCreationContainer cityCreationContainer = new CityCreationContainer()
								.setExternalId(cityId).setCountry(israel)
								.setName(streetEntry.getCityName());
						if (city == null) {
							city = cityService.createCityNoMerge(
									cityCreationContainer, securityContext);
							toMerge.add(city);
							existingCities.put(cityId, city);
							addressImportResponse
									.setCreatedCities(addressImportResponse
											.getCreatedCities() + 1);
						} else {
							if (cityService.updateCityNoMerge(
									cityCreationContainer, city)) {
								toMerge.add(city);
								addressImportResponse
										.setUpdatedCities(addressImportResponse
												.getUpdatedCities() + 1);
							} else {
								addressImportResponse
										.setUnchangedCities(addressImportResponse
												.getUnchangedCities() + 1);

							}
						}

						String streetId = streetEntry.getStreetId() + "";
						Street street = existingStreets.computeIfAbsent(cityId,
								f -> new HashMap<>()).get(streetId);
						StreetCreationContainer streetCreationContainer = new StreetCreationContainer()
								.setExternalId(streetId).setCity(city)
								.setName(streetEntry.getStreetName());
						if (street == null) {
							street = streetService.createStreetNoMerge(
									streetCreationContainer, securityContext);
							toMerge.add(street);
							existingStreets.computeIfAbsent(cityId,
									f -> new HashMap<>()).put(streetId, street);
							addressImportResponse
									.setCreatedStreet(addressImportResponse
											.getCreatedStreet() + 1);
						} else {
							if (streetService.updateStreetNoMerge(street,
									streetCreationContainer)) {
								toMerge.add(street);
								addressImportResponse
										.setUpdatedStreet(addressImportResponse
												.getUpdatedStreet() + 1);
							} else {
								addressImportResponse
										.setUnchangedStreet(addressImportResponse
												.getUnchangedStreet() + 1);

							}
						}
					}

					Set<String> cities = Stream.of(data)
							.map(f -> f.getCityId() + "")
							.collect(Collectors.toSet());
					Map<String, Set<String>> streets = Stream.of(data).collect(
							Collectors.groupingBy(
									f -> f.getCityId() + "",
									Collectors.mapping(f -> f.getStreetId()
											+ "", Collectors.toSet())));

					List<City> citiesToDel = existingCities.values()
							.parallelStream()
							.filter(f -> !cities.contains(f.getExternalId()))
							.collect(Collectors.toList());
					List<Street> streetsToDel = existingStreets.values()
							.parallelStream().flatMap(f -> f.values().stream())
							.filter(f -> deleteStreet(f, streets))
							.collect(Collectors.toList());
					for (City city : citiesToDel) {
						city.setSoftDelete(true);
						toMerge.add(city);
						addressImportResponse
								.setDeletedCities(addressImportResponse
										.getDeletedCities() + 1);
					}
					...
					for (Street street : streetsToDel) {
						...
						toMerge.add(street);
						addressImportResponse
								.setDeletedStreet(addressImportResponse
										.getDeletedStreet() + 1);
