...
public class CountryService implements ICountryService {
    ...
    public ImportCountriesResponse importCountries(SecurityContext securityContext, ImportCountriesRequest addressImportRequest) {
        ImportCountriesResponse importCountriesResponse = new ImportCountriesResponse();
        try {

            HttpResponse<ImportedCountry[]> response = Unirest.get(Config.getCountriesImportUrl()).asObject(ImportedCountry[].class);
            if (response.getStatus() == 200) {
                ImportedCountry[] data = response.getBody();
                List<Object> toMerge = new ArrayList<>();
                Map<String, Country> existingCountries = listAllCountries(securityContext, new CountryFiltering()).parallelStream().filter(f -> f.getCountryCode() != null).collect(Collectors.toMap(f -> f.getCountryCode(), f -> f, (a, b) -> a));
                for (ImportedCountry datum : data) {
                    if (existingCountries.get(datum.getCode()) == null) {
                        CountryCreationContainer creationContainer = new CountryCreationContainer().setCountryCode(datum.getCode()).setName(datum.getName());
                        Country country = createCountryNoMerge(creationContainer, securityContext);
